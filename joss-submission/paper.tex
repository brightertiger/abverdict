...
\title{\textbf{abverdict}: A Comprehensive Python Library for A/B Testing and Experimentation Statistics}

\author{
    abverdict contributors\\
    \texttt{https://github.com/abverdict/abverdict}
}
...
\begin{abstract}
\textbf{abverdict} is an open-source Python library that provides a complete statistical toolkit for A/B testing and online experimentation. The library offers sample size calculation, power analysis, significance testing, Bayesian inference, sequential testing with early stopping, and business impact projections. Designed with both analysts and non-technical users in mind, abverdict includes a web-based user interface that makes statistical analysis accessible to marketing and product teams. The library addresses the growing need for rigorous experimentation practices in industry while maintaining ease of use.
\end{abstract}

\section*{Summary}

Online controlled experiments (A/B tests) have become the gold standard for data-driven decision making in technology companies \citep{kohavi2020trustworthy}. However, proper statistical analysis of experiments requires expertise that many practitioners lack. \textbf{abverdict} bridges this gap by providing:
...
Despite the proliferation of A/B testing tools, most solutions fall into two categories: expensive enterprise platforms or fragmented open-source libraries that require significant statistical knowledge. abverdict addresses several critical gaps:
...
Proper experiment planning is crucial for reliable results. abverdict provides sample size calculators for:
...
Analyzing results across user segments requires proper multiple comparison corrections. abverdict supports:
...
\midrule
\texttt{abverdict.effects.outcome.conversion} & Binary outcome analysis \\
\texttt{abverdict.effects.outcome.magnitude} & Continuous outcome analysis \\
\texttt{abverdict.effects.outcome.timing} & Time-to-event analysis \\
\texttt{abverdict.methods.bayesian} & Bayesian inference \\
\texttt{abverdict.methods.sequential} & Sequential testing \\
\texttt{abverdict.diagnostics} & Experiment health checks \\
\texttt{abverdict.segments} & Segment analysis \\
\texttt{abverdict.business} & Impact projections \\
\texttt{abverdict.planning} & Duration estimation \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{Feature} & \textbf{abverdict} & \textbf{statsmodels} & \textbf{scipy} & \textbf{abtest} \\
\midrule
...
abverdict includes a comprehensive test suite with 367+ tests covering:
...
Full documentation is available at \url{https://github.com/abverdict/abverdict}, including:
